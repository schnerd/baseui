...
import {ICON, TYPE, STATE_TYPE} from './constants';
...
class Select extends React.Component<PropsT, StatelessStateT> {
  ...
  onChange = (
    ...
    let selectedOptions;
    switch (type) {
      case STATE_TYPE.select:
        if (!selected) {
          const selectedOptions = multiple
            ? this.state.selectedOptions.slice()
            : [];
    ...
    return (
      <InputComponent
        ...
        overrides={{
          Input: {
            props: {
              onKeyUp: e => this.onChange(e, STATE_TYPE.keyUp),
            },
            component: Input,
          },
          ...
          After: () => (
            <SearchIcon
              onClick={e => this.onChange(e, STATE_TYPE.clearAll)}
              $type={ICON.CLEAR_ALL}
              src={
  ...
  getDropDown() {
    const {options = [], overrides, type, rows} = this.props;
